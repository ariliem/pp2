...
def name_checker(NAMEBOX):
    ...
    cnt = 0
    cur.execute(' SELECT * FROM suppliers.public.snake_scores ')
    data = cur.fetchall()
    ...
    while not done:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_i:
                cur.execute(f''' INSERT INTO suppliers.public.snake_scores("user", "user_score") VALUES( '{user_name}', '{game.score}'); ''')
        ...
        if(game.gameOver() == True): # if our game over returns true, we will end the game
            ...
            cur.execute(f''' INSERT INTO suppliers.public.snake_scores("username", "user_score") VALUES( '{user_name}', '{game.score}'); ''')
